...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                elif (insect.container) and (self.ant.container == False):
                    inside_insect = self.ant
                    self.ant = insect
                    self.ant.contain_ant(inside_insect)
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if (type(insect) != QueenAnt) or (QueenAnt.queenAntIndex[0] != insect):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=float('inf')):
        ...
        index = 0
        while k != hive:
            if len(k.bees) != 0 and index >= self.min_range and index <= self.max_range:
                return random_or_none(k.bees)
            index = index + 1
            k = k.entrance
        return None
        # END Problem 3B
    ...
    def action(self, colony):
        ...
        self.throw_at(self.nearest_bee(colony.hive, self.min_range, self.max_range), colony)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        # BEGIN Problem 4A
        if (self.armor - amount <= 0) or (type(self) != FireAnt):
            position = len(self.place.bees) - 1
            while position >= 0:
                self.place.bees[position].reduce_armor(self.damage)
                position -= 1
        if type(self) == FireAnt:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if target_bee != None:
            Ant.reduce_armor(target_bee, target_bee.armor)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        if self.queenAntIndex[0] != self:
            self.reduce_armor(self.armor)
            return
        ThrowerAnt.throw_at(self, ThrowerAnt.nearest_bee(self, colony.hive, 0, float('inf')),colony)
        position = self.place
        while position.exit != None:
            position = position.exit
            if (position.ant != None):
                if position.ant.container and position.ant.ant != None and (position.ant.ant not in self.double_index):
                    position.ant.ant.damage *= 2
                    self.double_index += [position.ant.ant]
                if (position.ant not in self.double_index):
                    position.ant.damage *= 2
                    self.double_index += [position.ant]
        # END Problem 9
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        Insect.reduce_armor(self, amount)
        if (self.queenAntIndex[0] == self) and (self.armor <= 0):
            bees_win()
        # END Problem 9
...
def make_slow(action, colony, duration):
...
def make_stun(action, colony, duration):
...
def apply_effect(effect, bee, duration, colony):
    ...
    def new_action(colony):
        bee.action = effect(bee.action, colony, duration)
        return bee.action
    new_action(colony)
    # END Problem EC
